...
function setup(app, options) {
  ...
  if (process.env.REDIS_HOST) {
    redisClient = redis.createClient(redisPort = process.env.REDIS_PORT, redisHost = process.env.REDIS_HOST);
    redisClient.auth(redisPassword = process.env.REDIS_PASSWORD);
  } else if (process.env.OPENREDIS_URL) {
    var redisUrl = parseUrl(process.env.OPENREDIS_URL);
    redisClient = redis.createClient(redisPort = redisUrl.port, redisHost = redisUrl.hostname);
    redisClient.auth(redisPassword = redisUrl.auth.split(":")[1]);
  } else {
  ...
  expressApp
    ...
    , store: new RedisStore({host: redisHost, port: redisPort, pass: redisPassword})
    }))
    .use(createUserId)
